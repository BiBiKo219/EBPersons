...
class SwinTransformer_DC5(BaseModule):
    """ Swin Transformer
    ...
    def __init__(self,
                 ...
                 frozen_stages=-1,
                 init_cfg=None):  # strides dilations
        ...
        super(SwinTransformer_DC5, self).__init__(init_cfg=init_cfg)
        ...
        for i in range(num_layers):
            if i < num_layers - 1:
                downsample = PatchMerging(
                    ...
                    stride=strides[i + 1],    # 类似resnet dc5，最后一层stride = 1
                    dilation=dilations[i + 1],   # 最后一块dilations = 2
                    norm_cfg=norm_cfg if patch_norm else None,
                    init_cfg=None)
            else:
                downsample = None
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformer_DC5, self).train(mode)
        self._freeze_stages()
